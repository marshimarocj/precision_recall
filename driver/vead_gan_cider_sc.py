...
if __name__ == '__main__':
  ...
  if opts.is_train:
    trntst = model.vead_gan_cider_sc.TrnTst(model_cfg, path_cfg, [0])
    # trn_reader = model.data.AttTrnGanReader(path_cfg.trn_ftfile, path_cfg.trn_att_ftfile, path_cfg.trn_videoid_file, path_cfg.trn_annotation_file)
    trn_reader = model.data.AttTrnGanReader(path_cfg.val_ftfile, path_cfg.val_att_ftfile, path_cfg.val_videoid_file, path_cfg.val_annotation_file)
    ...
    model_cfg.subcfgs[model.vead_gan_cider_sc.DEC].beam_width = opts.beam_width
