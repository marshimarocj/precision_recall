...
def build_parser():
  ...
  parser.add_argument('--fix_lr', dest='fix_lr', type=int, default=True)
...
if __name__ == '__main__':
  ...
  if opts.is_train:
    if opts.fix_lr:
      trntst = model.vevd_ml.TrnTstFixLr(model_cfg, path_cfg, gpuids)
    else:
      trntst = model.vevd_ml.TrnTst(model_cfg, path_cfg, gpuids)
