...
class Discriminator(nn.Module):
  ...
  def forward(self, mode, fts, sents, lens, **kwargs):
    ...
    if mode == 'trn':
      ...
      logits, logits_img, logits_sent, greedy, greedy_img, greedy_sent = \
        self.predict_with_greedy(fts, sents, lens, kwargs['greedy_sents'], kwargs['greedy_lens'])
      log_p = F.log_softmax(logits, dim=-1)
      log_p_img = F.log_softmax(logits_img, dim=-1)
      log_p_sent = F.log_softmax(logits_sent, dim=-1)
      log_greedy = F.log_softmax(greedy, dim=-1)
      log_greedy_img = F.log_softmax(greedy_img, dim=-1)
      log_greedy_sent = F.log_softmax(greedy_sent, dim=-1)
      return log_p[:, 1], log_p_img[:, 1], log_p_sent[:, 1], log_greedy[:, 1], log_greedy_img[:, 1], log_greedy_sent[:, 1]
    else:
      logits, logits_img, logits_sent, dist2img, dist2sent = self.predict(fts, sents, lens)
  ...
  def predict_with_greedy(self, fts, sents, lens, greedy_sents, greedy_lens):
    b = fts.size(0)
    num_sentence = self._config.num_sentence
    dim_kernel = self._config.dim_kernel
    num_kernel = self._config.num_kernel
    ...
    # greedy
    greedy_hidden = self.sent_encoder(greedy_sents, greedy_lens)
    greedy_sent2img_embed = self.sent_img_embed(greedy_hidden)
    greedy_sent2img_embed = F.dropout(greedy_sent2img_embed, p=self._config.noise).unsqueeze(1) # (b, 1, num_kernel*dim_kernel)
    greedy_sent2sent_embed = self.sent_sent_embed(greedy_hidden)
    greedy_sent2sent_embed = F.dropout(greedy_sent2sent_embed, p=self._config.noise).unsqueeze(1) # (b, 1, num_kernel*dim_kernel)

    greedy_dist2img = (img_embed * greedy_sent2img_embed).view(b, num_kernel, dim_kernel)
    greedy_dist2img = torch.sum(greedy_dist2img, -1) / (dim_kernel**.5)
    greedy_dist2img = F.tanh(greedy_dist2img) # (b, num_kernel)

    greedy_dist2sent = (sent2sent_embed * greedy_sent2sent_embed).view(b, num_sentence, num_kernel, dim_kernel)
    greedy_dist2sent = torch.sum(greedy_dist2sent, -1) / (dim_kernel**.5)
    greedy_dist2sent = torch.mean(F.tanh(greedy_dist2sent), 1) # (b, num_kernel)

    greedy_img = self.ff_img(greedy_dist2img)
    greedy_sent = self.ff_sent(greedy_dist2sent)
    greedy = greedy_img + greedy_sent
    # print greedy_img.size(), greedy_sent.size(), greedy.size()

    return out, out_img, out_sent, greedy, greedy_img, greedy_sent
